...
clinical$age_category <- ifelse(clinical$age_at_initial_pathologic_diagnosis < 50, clinical$age_category <- "young", clinical$age_category <- "old")
...
age_fit <- survfit(surv_object_age ~ clinical$age_category,
                     data = clinical)
...
Take a moment to look at your plot and try to understand what it is saying. Write some observations/key takeaways below: Old and young people have similar rates of survival in the early days of diagnosis. As time goes on, younger people more often pass away earlier in their prognosis.
...
write.csv(clinical, "/Users/joshuagabbay/Desktop/QBIO/qbio_490_joshuagabbay/brca_clinical_data.csv", row.names = FALSE)
```
